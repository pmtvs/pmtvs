...
**Result:** 63/63 PASS. Reproduce: `python benchmarks/parity_report.py`

### BENCHED Functions — Not Yet Validated

The following functions have Rust implementations that compile and run
but have NOT passed parity validation. They ship as Python-only until
validated. We disclose their existence and benchmark numbers for
transparency.

| Function | Rust Speedup | Parity Status | Reason |
|----------|-------------|---------------|--------|
| `lyapunov_rosenstein` | 2,776x | SKIP | Output divergence exceeds relaxed tolerance |
| `lyapunov_kantz` | 1,437x | SKIP | Output divergence exceeds relaxed tolerance |
| `ftle_local_linearization` | 8.5x | SKIP | Parity test not yet written |
| `ftle_direct_perturbation` | 46.5x | SKIP | Parity test not yet written |
| `dynamic_time_warping` | 121x | SKIP | Parity test not yet written |
| `psd` | 16.4x | SKIP | Parity test not yet written |
| `dominant_frequency` | 16.6x | SKIP | Parity test not yet written |
| `spectral_centroid` | 16.0x | SKIP | Parity test not yet written |
| `spectral_bandwidth` | 31.0x | SKIP | Parity test not yet written |
| `spectral_entropy` | 16.8x | SKIP | Parity test not yet written |
| `eigendecomposition` | 3.8x | SKIP | LAPACK reference preferred |
| `svd` | 0.8x | SKIP | LAPACK faster |
| `distance_matrix` | 0.015x | SKIP | NumPy vectorization faster |

**These functions exist in compiled Rust but are NOT registered in
the dispatch table. Setting `PMTVS_USE_RUST=1` does NOT activate them.
They remain dormant until parity is proven.**

Speed without accuracy is a faster piece of crap.
