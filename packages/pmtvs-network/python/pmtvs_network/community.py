...
def modularity(
    ...
    resolution : float
        Resolution parameter (gamma). Higher values produce more communities.
    ...
    c_i = community of node i, delta = Kronecker delta.
    """
    ...
    # Compute modularity using vectorized operations
    Q = 0.0
    ...
    for c in unique_comms:
        ...
        A_c = np.sum(adjacency[np.ix_(mask, mask)])
        # Sum of degrees in community
        k_c = np.sum(k[mask])
        Q += A_c / (2.0 * m) - resolution * (k_c / (2.0 * m)) ** 2
...
def community_detection(
    ...
    n_communities : int, optional
        Number of communities (required for spectral, ignored otherwise)

    Returns
    -------
    tuple
        (communities, modularity_score)
        - communities: np.ndarray of integer community labels
        - modularity_score: float modularity of the partition
